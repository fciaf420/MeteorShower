...
/**
 * Scans wallet for accumulated tokens that can be used in rebalancing
 * @param {Connection} connection - Solana connection
 * @param {PublicKey} walletPubKey - User's wallet public key
 * @param {PublicKey} tokenXMint - Token X mint address
 * @param {PublicKey} tokenYMint - Token Y mint address (SOL)
 * @returns {Object} Available wallet tokens for each mint
 */
async function getWalletTokensForRebalancing(connection, walletPubKey, tokenXMint, tokenYMint) {
  try {
    console.log('🔍 Scanning wallet for accumulated tokens...');
    ...
    console.log(`   💰 Wallet Token X: ${tokenXAmount.toFixed(6)}`);
    console.log(`   💰 Wallet SOL: ${tokenYAmount.toFixed(6)}`);
    ...
    console.error('⚠️  Error scanning wallet tokens:', error.message);
    return {
