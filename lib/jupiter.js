...
async function executeUltraOrder(order, userKeypair, connection, dlmmPool, maxAttempts = 20) {
  ...
  while (attempt < maxAttempts) {
    attempt += 1;
    console.log(`\n[executeUltraOrder] attempt ${attempt}/${maxAttempts}`);
    ...
    try {
      ...
      console.error(`❌ Ultra API execution error (attempt ${attempt}):`, error.message);
      
      if (attempt >= maxAttempts) {
        console.error('❌ All Ultra API attempts exhausted. Returning null.');
        return null;
      }
...
async function swapTokensUltra(
  ...
  try {
    ...
    if (!order) {
      console.log('❌ Could not get Ultra API order');
      return null;
    }
    
    // Step 2: Execute the order
    const signature = await executeUltraOrder(order, userKeypair, connection, dlmmPool, maxAttempts);
    
    if (signature) {
      console.log('✅ Ultra API swap completed successfully!');
      return signature;
    } else {
      console.log('❌ Ultra API swap failed - no fallback, operation failed');
      return null;
    }
    
  } catch (error) {
    console.error('❌ Error in Ultra API swap:', error.message);
    console.log('❌ Ultra API operation failed - no fallback, returning null');
    return null;
  }
}
