...
// ────────────────────────────────────────────────────────────────────────────
// Helpers: dynamic slippage computation and clamps
// ────────────────────────────────────────────────────────────────────────────
function clamp(num, min, max) {
  return Math.max(min, Math.min(max, num));
}

function recommendSlippageBpsFromQuote(quote) {
  // Jupiter returns priceImpactPct as a fraction (e.g. 0.0123 = 1.23%)
  const impactFraction = Number(quote?.priceImpactPct) || 0;
  const impactBps = impactFraction * 10000; // convert to bps
  const hopCount = Array.isArray(quote?.routePlan) ? quote.routePlan.length : 1;

  // Base = 1.5x impact, plus 10 bps per extra hop (max +80 bps)
  const base = impactBps * 1.5;
  const hopBuffer = Math.min(Math.max(hopCount - 1, 0) * 10, 80);
  const rec = Math.ceil(base + hopBuffer);

  // Guardrails: at least 30 bps, cap at 500 bps
  return clamp(rec, 30, 500);
}

function looksLikeSlippageError(message = "") {
  const m = message.toLowerCase();
  return (
    m.includes('slippage') ||
    m.includes('otheramountthreshold') ||
    m.includes('price changed') ||
    m.includes('exceeded slippage')
  );
}
...
async function getSwapQuote(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      // Prefer stable routes; avoid illiquid random hops
      ...
      // If our initial slippage is below what we recommend for this route,
      // fetch a better quote using the recommended slippage once.
      const recBps = recommendSlippageBpsFromQuote(quote);
      if (recBps > slippageBps) {
        console.log(`Re-quoting with dynamic slippage: ${slippageBps} → ${recBps} bps`);
        const url2 = new URL("https://lite-api.jup.ag/swap/v1/quote");
        url2.searchParams.set("inputMint", inputMint);
        url2.searchParams.set("outputMint", outputMint);
        url2.searchParams.set("amount", amountRaw.toString());
        url2.searchParams.set("slippageBps", recBps.toString());
        url2.searchParams.set("restrictIntermediateTokens", "true");
...
async function executeSwap(quoteResponse, userKeypair, connection, dlmmPool, maxAttempts = 20) {
  ...
  let currentMaxBps = recommendSlippageBpsFromQuote(currentQuote);
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      const buildRes = await fetch("https://lite-api.jup.ag/swap/v1/swap", {
        ...
        body: JSON.stringify({
          ...
          dynamicSlippage: { maxBps: currentMaxBps },
    ...
    try {
      ...
      try {
        ...
        // Batch fetch prices to avoid multiple API calls
        const prices = await getPrices([inMint, outMint]);
        const inPrice = prices[inMint] || 0;
        const outPrice = prices[outMint] || 0;
        
        const inUsd = inUi * inPrice;
        const outUsd = outUi * outPrice;
        const diff = inUsd - outUsd
        const slipUsd = Number(diff) / 10**outDecs * outPrice;
      ...
      if (attempt < maxAttempts) {
        // Auto-bump slippage for next attempt if error suggests slippage
        if (looksLikeSlippageError(err.message)) {
          const bumped = clamp(currentMaxBps + 20, 30, 500);
          if (bumped !== currentMaxBps) {
            console.log(`[executeSwap] slippage-related error → bumping maxBps ${currentMaxBps} → ${bumped}`);
            currentMaxBps = bumped;
          }
        }
        console.log("[executeSwap] fetching a fresh quote before next retry...");
        await new Promise((r) => setTimeout(r, 500));
        currentQuote = await getSwapQuote(inMint, outMint, inAmountRaw, currentMaxBps);
        if (!currentQuote) {
          console.error(
...
async function swapTokensUltra(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt >= maxAttempts) {
        ...
        break; // fall through to Lite API fallback below
      }
  ...
  // ──────────────────────────────────────────────────────────────
  // Fallback: Lite Quote + Swap API
  // ──────────────────────────────────────────────────────────────
  try {
    console.log("⚠️  Falling back to Jupiter Lite Quote/Swap API...");
    // brief wait for balances to index
    await new Promise(r => setTimeout(r, 1000));
    const quote = await getSwapQuote(inputMint, outputMint, amountRaw, slippageBps, 5 /* attempts */);
    if (!quote) {
      console.error("Lite Quote failed – giving up.");
      return null;
    }
    const sig = await executeSwap(quote, userKeypair, connection, dlmmPool, 3);
    if (sig) {
      console.log("✅ Lite Swap API fallback succeeded:", sig);
      return sig;
    }
    console.error("Lite Swap API fallback did not land.");
    return null;
  } catch (e) {
    console.error("❌ Fallback swap error:", e.message);
    return null;
  }
}
