...
export async function getDynamicPriorityFee(connection, transaction, priorityLevel = PRIORITY_LEVELS.MEDIUM, extraOptions = {}) {
  ...
  if (rpcProvider !== 'helius') {
    console.log(`📡 RPC Provider: ${rpcProvider} (${connection.rpcEndpoint.split('//')[1]?.split('/')[0] || 'unknown'})`);
    console.log(`📋 Using static priority fee: ${FALLBACK_FEES[priorityLevel].toLocaleString()} micro-lamports`);
    console.log(`💡 For dynamic fees, consider using Helius RPC`);
