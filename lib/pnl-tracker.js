...
export class PnLTracker {
  constructor() {
    ...
    this.claimedFees = { sol: new BN(0), token: new BN(0) }; // Claimed fees
  ...
  async getBinLevelBreakdown(position, pool, userPublicKey = null) {
    console.log('🔍 Starting bin-level breakdown analysis...');
    ...
    try {
      ...
      console.log('📊 Fetching user positions from pool...');
      ...
      console.log(`📊 Processing ${binData.length} bins from position data`);
      ...
      if (!hasAnyLiquidity) {
        console.log('⚠️ Position appears to be empty or being closed - skipping bin analysis');
      ...
      for (const bin of binData) {
        ...
        if (!solAmount.isZero() || !tokenAmount.isZero()) {
          ...
          console.log(`  Bin ${bin.binId}: ${this.lamportsToSol(solAmount).toFixed(6)} SOL, ${this.tokenAmountToReadable(tokenAmount, tokenDecimals)} Token`);
      ...
      console.log(`⚠️ SDK bin breakdown failed: ${error.message}`);
      console.log('⚠️ Available position properties:', Object.keys(position));
      
      // Fallback: Use position totals if available
      console.log('⚠️ Using position totals as fallback');
    ...
    console.log(`✅ Bin breakdown complete: ${bins.length} bins processed`);
  ...
  async calculatePnL(position, pool, currentSolPrice = null, currentTokenPrice = null, newFees = null, userPublicKey = null) {
    ...
    // Update claimed fees if new fees provided
    if (newFees) {
      this.claimedFees.sol = this.claimedFees.sol.add(newFees.sol || new BN(0));
      this.claimedFees.token = this.claimedFees.token.add(newFees.token || new BN(0));
    }
  ...
  importState(state) {
    ...
    this.totalFeesEarned = new BN(state.totalFeesEarned || '0');
