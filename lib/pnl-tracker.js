...
export class PnLTracker {
    ...
    try {
      ...
      for (const bin of binData) {
        ...
        if (!solAmount.isZero() || !tokenAmount.isZero()) {
          ...
          // 🔧 FIX: Use standardized fee mapping instead of inline logic
          const feeMapped = mapDlmmFees(feeX, feeY, X_IS_SOL);
          const claimableSolFee = feeMapped.solFee;
          const claimableTokenFee = feeMapped.tokenFee;
   ...
   */
  async calculatePnL(position, pool, currentSolPrice = null, currentTokenPrice = null, newFees = null, userPublicKey = null, currentBaseline = null) {
    if (!this.baseline) {
    ...
    // Calculate percentages - use current baseline if provided, otherwise fall back to initial deposit
    const effectiveBaseline = currentBaseline || this.initialDeposit;
    const absolutePnL = totalCurrentValue - effectiveBaseline;
    const absolutePnLPercent = (absolutePnL / effectiveBaseline) * 100;

    // Debug logging for baseline usage (only when baseline is passed)
    if (currentBaseline !== null && Math.abs(currentBaseline - this.initialDeposit) > 1) {
      console.log(`🔧 [P&L Fix] Using updated baseline: $${effectiveBaseline.toFixed(2)} (initial: $${this.initialDeposit.toFixed(2)})`);
    }
