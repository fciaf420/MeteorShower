...
async function analyzeAltTokens(dlmmPool, balances, tokenXMint, tokenYMint) {
  ...
  if (tokenXMint !== SOL_MINT.toString() && !balances.tokenX.isZero()) {
    ...
    altTokenAmount = balances.tokenX.toNumber() / Math.pow(10, dlmmPool.tokenX.decimal);
  ...
  // Check token Y
    ...
    altTokenAmount = balances.tokenY.toNumber() / Math.pow(10, dlmmPool.tokenY.decimal);
