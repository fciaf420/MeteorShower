...
export function verifyTokenAssignments(dlmmPool) {
  ...
  // Debug log to understand the comparison
  const xStr = tokenXKey.toString();
  const yStr = tokenYKey.toString();
  ...
  return {
    ...
    explanation: isCorrectOrder
      ? 'Token assignments follow typical SDK ordering (X < Y lexicographically)'
      : `Token assignments use alternative ordering (X > Y lexicographically) - pool functions normally`
  };
}
...
export function logTokenAssignments(dlmmPool, label = 'Token Assignment Analysis') {
  ...
  if (!analysis.isCorrectOrder) {
    console.log(`   ℹ️ NOTE: Token order differs from typical SDK sorting (this is cosmetic - bot continues normally)`);
  }
}
