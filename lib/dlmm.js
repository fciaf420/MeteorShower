...
async function recenterPosition(connection, dlmmPool, userKeypair, oldPositionPubKey, originalParams = {}, rebalanceDirection = null) {
  ...
  try {
    ...
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      ...
      }, exactBalances, true); // Skip existing position check
    } else {
      ...
      openRes = await openDlmmPosition(connection, userKeypair, capitalToUse, tokenRatio, binSpan, poolAddress, liquidityStrategy, null, exactBalances, true); // Skip existing position check
    }
