...
async function openDlmmPosition(connection, userKeypair, solAmount = null, tokenRatio = null, binSpan = null, poolAddress = null, liquidityStrategy = null, swaplessOptions = null, providedBalances = null) {
...
async function closeDlmmPosition(connection, dlmmPool, userKeypair, positionPubKey) {
    return await withRetry(async () => {
      ...
      const instructions = Array.isArray(closeIx) ? closeIx[0].instructions : closeIx.instructions;
...
async function recenterPosition(connection, dlmmPool, userKeypair, oldPositionPubKey, originalParams = {}, rebalanceDirection = null) {
  ...
  // 🎯 SMART WALLET-AWARE AUTO-COMPOUNDING
  ...
  if (autoCompoundConfig && autoCompoundConfig.enabled) {
    console.log(`✅ Smart auto-compounding enabled`);
    
    // Get wallet token balances for smart compounding
    // SAFETY: This only scans for tokens matching the exact LP pair mints
    // LIMIT: Pass initial deposit to prevent wallet drainage
    const initialDepositUsd = originalParams.solAmount ? originalParams.solAmount * 168 : 0; // Rough USD estimate
    walletTokens = await getWalletTokensForRebalancing(
      connection, 
    ...
    // 🔒 SAFETY VERIFICATION: Confirm we're only using LP pair tokens
    console.log(`🔒 Safety check: Verifying token mints match LP pair`);
    console.log(`   LP Token X: ${dlmmPool.tokenX.publicKey.toBase58()}`);
    console.log(`   LP Token Y: ${dlmmPool.tokenY.publicKey.toBase58()}`);
    console.log(`   Wallet scan results: ${walletTokens.tokenXMint} | ${walletTokens.tokenYMint}`);
    
    if (walletTokens.limitApplied) {
      console.log(`   🛡️  SAFETY LIMITS APPLIED to prevent wallet drainage`);
      console.log(`      Original SOL available: ${walletTokens.originalTokenYAmount.toFixed(6)}`);
      console.log(`      Limited to: ${walletTokens.tokenYAmount.toFixed(6)} (based on initial deposit)`);
    }
    
    // 🔍 DETECT WHICH TOKEN IS SOL
    const SOL_MINT = 'So11111111111111111111111111111111111111112';
    const X_IS_SOL = dlmmPool.tokenX.publicKey.toBase58() === SOL_MINT;
    const Y_IS_SOL = dlmmPool.tokenY.publicKey.toBase58() === SOL_MINT;
    
    console.log(`🔍 Token identification: X_IS_SOL=${X_IS_SOL}, Y_IS_SOL=${Y_IS_SOL}`);
    
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      console.log(`🎯 Smart compounding for ${rebalanceDirection} direction:`);
      
      if (rebalanceDirection === 'UP') {
        // UP = SOL-only position: Use all SOL sources, preserve alt token for future
        if (X_IS_SOL) {
          // SOL is Token X - use it for UP position
          lamX = lamX.add(feeX); // SOL fees into position  
          lamY = new BN(0); // Don't use alt token fees now
          console.log(`   📍 SOL is Token X - using for UP position`);
        } else if (Y_IS_SOL) {
          // SOL is Token Y - use it for UP position  
          lamY = lamY.add(feeY); // SOL fees into position
          lamX = new BN(0); // Don't use alt token fees now
          console.log(`   📍 SOL is Token Y - using for UP position`);
        }
        
        // 🔒 Add accumulated SOL from wallet based on which token is SOL
        const SOL_BUFFER = new BN(process.env.SOL_FEE_BUFFER_LAMPORTS || 70_000_000);
        
        if (X_IS_SOL && walletTokens.tokenXMint === dlmmPool.tokenX.publicKey.toBase58()) {
          // SOL is Token X - add from wallet Token X
          const usableWalletSOL = walletTokens.walletTokenX.gt(SOL_BUFFER) 
            ? walletTokens.walletTokenX.sub(SOL_BUFFER) 
            : new BN(0);
          
          if (usableWalletSOL.gt(new BN(0))) {
            lamX = lamX.add(usableWalletSOL);
            console.log(`   💡 Added ${(usableWalletSOL.toNumber() / 10 ** dx).toFixed(6)} accumulated SOL from wallet (Token X)`);
            console.log(`   🔒 Verified SOL mint: ${walletTokens.tokenXMint.slice(0,8)}...`);
          }
        } else if (Y_IS_SOL && walletTokens.tokenYMint === dlmmPool.tokenY.publicKey.toBase58()) {
          // SOL is Token Y - add from wallet Token Y
          const usableWalletSOL = walletTokens.walletTokenY.gt(SOL_BUFFER) 
            ? walletTokens.walletTokenY.sub(SOL_BUFFER) 
            : new BN(0);
          
          if (usableWalletSOL.gt(new BN(0))) {
            lamY = lamY.add(usableWalletSOL);
            console.log(`   💡 Added ${(usableWalletSOL.toNumber() / 10 ** dy).toFixed(6)} accumulated SOL from wallet (Token Y)`);
            console.log(`   🔒 Verified SOL mint: ${walletTokens.tokenYMint.slice(0,8)}...`);
          }
        }
        
        console.log(`   ✅ SOL-only position: Using SOL fees + wallet SOL`);
        if (X_IS_SOL) {
          console.log(`   💾 Alt token fees (${(feeY.toNumber() / 10 ** dy).toFixed(6)}) preserved in wallet for future use`);
        } else {
          console.log(`   💾 Alt token fees (${(feeX.toNumber() / 10 ** dx).toFixed(6)}) preserved in wallet for future use`);
        }
        
      } else if (rebalanceDirection === 'DOWN') {
        // DOWN = Alt token-only position: Use all alt token sources, preserve SOL for future
        if (X_IS_SOL) {
          // SOL is Token X, so alt token is Token Y - use Token Y for DOWN position
          lamY = lamY.add(feeY); // Alt token fees into position
          lamX = new BN(0); // Don't use SOL fees now
          console.log(`   📍 Alt token is Token Y - using for DOWN position`);
          
          // Add accumulated alt token from wallet
          if (walletTokens.tokenYMint === dlmmPool.tokenY.publicKey.toBase58() && walletTokens.walletTokenY.gt(new BN(0))) {
            lamY = lamY.add(walletTokens.walletTokenY);
            console.log(`   💡 Added ${walletTokens.tokenYAmount.toFixed(6)} accumulated alt token from wallet (Token Y)`);
            console.log(`   🔒 Verified alt token mint: ${walletTokens.tokenYMint.slice(0,8)}...`);
          }
          
        } else if (Y_IS_SOL) {
          // SOL is Token Y, so alt token is Token X - use Token X for DOWN position
          lamX = lamX.add(feeX); // Alt token fees into position
          lamY = new BN(0); // Don't use SOL fees now
          console.log(`   📍 Alt token is Token X - using for DOWN position`);
          
          // Add accumulated alt token from wallet
          if (walletTokens.tokenXMint === dlmmPool.tokenX.publicKey.toBase58() && walletTokens.walletTokenX.gt(new BN(0))) {
            lamX = lamX.add(walletTokens.walletTokenX);
            console.log(`   💡 Added ${walletTokens.tokenXAmount.toFixed(6)} accumulated alt token from wallet (Token X)`);
            console.log(`   🔒 Verified alt token mint: ${walletTokens.tokenXMint.slice(0,8)}...`);
          }
        }
        
        console.log(`   ✅ Alt token-only position: Using alt token fees + wallet alt tokens`);
        if (X_IS_SOL) {
          console.log(`   💾 SOL fees (${(feeX.toNumber() / 10 ** dx).toFixed(6)}) preserved in wallet for future use`);
        } else {
          console.log(`   💾 SOL fees (${(feeY.toNumber() / 10 ** dy).toFixed(6)}) preserved in wallet for future use`);
        }
      }
      
    } else {
      // Normal auto-compounding (non-swapless)
    lamX = lamX.add(feeX);
    lamY = lamY.add(feeY);
      console.log(`✅ Standard auto-compounding: All fees added to position capital`);
    }
  ...
  try {
    ...
    // Prepare exact token amounts from closed position + smart compounding
    const exactBalances = {
      lamX: lamX,  // Smart compounded amounts (position + compatible fees + wallet tokens)
      lamY: lamY
    };
    
    // Enhanced logging for smart compounding
    const finalTokenXAmount = lamX.toNumber() / 10 ** dx;
    const finalTokenYAmount = lamY.toNumber() / 10 ** dy;
    console.log(`🎯 Smart compounding result:`);
    ...
    if (walletTokens) {
      console.log(`   📊 Capital efficiency: Using accumulated wallet tokens + position tokens + compatible fees`);
    }
    
    // Check if swapless rebalancing is enabled
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      console.log(`🔄 [recenter] Swapless rebalancing - Direction: ${rebalanceDirection}, Using original bin span: ${binSpan} bins`);
      console.log(`🔄 [recenter] Using exact balances from closed position: ${lamX.toNumber() / 10**dx} X, ${lamY.toNumber() / 10**dy} Y`);
      
      // Pass exact balances to prevent double-counting
      openRes = await openDlmmPosition(connection, userKeypair, capitalToUse, tokenRatio, binSpan, poolAddress, liquidityStrategy, {
        swapless: true,
        direction: rebalanceDirection,
        swaplessSpan: binSpan  // Use original bin span, not swapless span
      }, exactBalances);
    } else {
      ...
      // Pass exact balances for normal rebalancing too
      openRes = await openDlmmPosition(connection, userKeypair, capitalToUse, tokenRatio, binSpan, poolAddress, liquidityStrategy, null, exactBalances);
    }
