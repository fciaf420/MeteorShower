    ...
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      ...
      if (rebalanceDirection === 'UP') {
        ...
        if (usableWalletSOL.gt(new BN(0))) {
          ...
          console.log(`   💡 Added ${(usableWalletSOL.toNumber() / 10 ** dx).toFixed(6)} accumulated SOL from wallet`);
        }
        ...
        if (walletTokens.walletTokenY.gt(new BN(0))) {
          ...
          console.log(`   💡 Added ${walletTokens.tokenYAmount.toFixed(6)} accumulated Token Y from wallet`);
        }
