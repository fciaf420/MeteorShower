...
async function recenterPosition(connection, dlmmPool, userKeypair, oldPositionPubKey, originalParams = {}, rebalanceDirection = null) {
  ...
  const { autoCompoundConfig } = originalParams;
  if (autoCompoundConfig && autoCompoundConfig.enabled) {
    // Auto-compound: Add fees to position capital
    lamX = lamX.add(feeX);
    lamY = lamY.add(feeY);
    console.log(`✅ Auto-compounding enabled: Fees added to position capital`);
  } else {
    // Keep fees separate: Fees will be claimed but not added to new position
  ...
  try {
    ...
    // Prepare exact token amounts from closed position
    const exactBalances = {
      lamX: lamX,  // Exact tokens from closed position (+ fees if auto-compound)
      lamY: lamY
    };
