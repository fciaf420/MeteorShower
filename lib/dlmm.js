  ...
  if (autoCompoundConfig && autoCompoundConfig.enabled) {
    ...
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      ...
      if (rebalanceDirection === 'UP') {
        // UP = SOL-only position: Use all SOL sources, preserve Token Y for future
        // Note: In most pools, SOL is TokenX and other token is TokenY
        lamX = lamX.add(feeX); // SOL fees into position  
        lamY = new BN(0); // Don't use Token Y fees now
        
        // 🔒 SAFETY: Only use wallet SOL if it matches the LP pair Token X mint
        if (walletTokens.tokenXMint === dlmmPool.tokenX.publicKey.toBase58()) {
          // Add accumulated SOL from wallet (minus fee buffer)
          ...
          if (usableWalletSOL.gt(new BN(0))) {
            ...
            console.log(`   💡 Added ${(usableWalletSOL.toNumber() / 10 ** dx).toFixed(6)} accumulated LP Token X from wallet`);
            console.log(`   🔒 Verified mint: ${walletTokens.tokenXMint.slice(0,8)}...`);
          }
        } else {
          console.log(`   ⚠️  Skipping wallet Token X: Mint mismatch (safety protection)`);
        }
        
        console.log(`   ✅ SOL-only position: Using SOL fees + wallet SOL`);
        console.log(`   💾 Token Y fees (${(feeY.toNumber() / 10 ** dy).toFixed(6)}) preserved in wallet for future use`);
        
      } else if (rebalanceDirection === 'DOWN') {
        // DOWN = Token-only position: Use all Token Y sources, preserve SOL for future
        lamY = lamY.add(feeY); // Token Y fees into position
        lamX = new BN(0); // Don't use SOL fees now
        
        // 🔒 SAFETY: Only use wallet Token Y if it matches the LP pair Token Y mint
        if (walletTokens.tokenYMint === dlmmPool.tokenY.publicKey.toBase58()) {
          // Add ALL accumulated Token Y from wallet  
          if (walletTokens.walletTokenY.gt(new BN(0))) {
            lamY = lamY.add(walletTokens.walletTokenY);
            console.log(`   💡 Added ${walletTokens.tokenYAmount.toFixed(6)} accumulated LP Token Y from wallet`);
            console.log(`   🔒 Verified mint: ${walletTokens.tokenYMint.slice(0,8)}...`);
          }
        } else {
          console.log(`   ⚠️  Skipping wallet Token Y: Mint mismatch (safety protection)`);
        }
        
        console.log(`   ✅ Token-only position: Using Token Y fees + wallet Token Y`);
