  ...
  const executePositionCreation = async (slippage, attemptNumber, priorityLevel) => {
    ...
    try {
      ...
      if (!skipExistingCheck) {
        ...
        if (existingPosition) {
          ...
          // If this is a retry attempt (likely due to signature expiration),
          // return the existing position instead of creating a new one
          if (attemptNumber > 0) {
            console.log(`💡 Retry detected - using existing position (likely created by previous "failed" attempt)`);
            return {
              dlmmPool,
              positionPubKey: existingPosition.publicKey,
              signature: null // No new signature since we're using existing position
            };
          } else {
    ...
    const targetSolLamportsBN = (
      ...
      if (solAmount !== null && Number.isFinite(solAmount)) {
        ...
        if (X_IS_SOL) {
          if (isRebalancing) {
            // 🔧 REBALANCING: Use ALL available SOL minus minimal tx buffer
            const minimalTxBuffer = new BN(0.01 * 1e9); // 0.01 SOL for transactions
            const maxSolToUse = nativeBalance.sub(minimalTxBuffer);
            lamX = BN.min(lamX, maxSolToUse);
            console.log(`🔄 REBALANCING: Using maximum SOL (Token X): ${lamX.toNumber() / 1e9} SOL (full capital allocation)`);
          } else {
            // INITIAL POSITION: Apply budget limits and buffer
        ...
        if (Y_IS_SOL) {
          if (isRebalancing) {
            ...
            console.log(`🔄 REBALANCING: Using maximum SOL (Token Y): ${lamY.toNumber() / 1e9} SOL (full capital allocation)`);
          } else {
            // INITIAL POSITION: Apply budget limits and buffer
            const maxSolToUse = BN.min(solAmountLamports, nativeBalance.sub(SOL_BUFFER));
            lamY = BN.min(lamY, maxSolToUse);
      ...
      if (nativeBalance.lte(SOL_BUFFER)) {
        ...
        if (attemptNumber > 0) {
          console.log(`💡 HINT: This may indicate a previous transaction succeeded despite appearing to fail.`);
          console.log(`💡 Check your wallet for existing positions before retrying.`);
        }
