...
async function openDlmmPosition(connection, userKeypair, solAmount = null, tokenRatio = null, binSpan = null, poolAddress = null, liquidityStrategy = null, swaplessOptions = null, providedBalances = null, skipExistingCheck = false) {
  return await withRetry(async () => {
    ...
    if (X_IS_SOL) {
      ...
      // CRITICAL FIX: Even during rebalancing, reserve some SOL for transaction fees and rent
      const REBALANCE_BUFFER = new BN(30_000_000); // 0.03 SOL for rebalancing transaction fees
      
      console.log(`📊 [REBALANCE-LOG] Applying Rebalancing Buffer Reservation:`);
      console.log(`   • Buffer Amount: ${REBALANCE_BUFFER.toNumber() / 1e9} SOL (${REBALANCE_BUFFER.toString()} lamports)`);
      console.log(`   • X_IS_SOL: ${X_IS_SOL}`);
      console.log(`   • Y_IS_SOL: ${Y_IS_SOL}`);
      console.log(`   • Pre-Buffer lamX: ${lamX.toString()} (${(lamX.toNumber() / 10**dx).toFixed(6)} tokens)`);
      console.log(`   • Pre-Buffer lamY: ${lamY.toString()} (${(lamY.toNumber() / 10**dy).toFixed(6)} tokens)`);
      
      // 🎯 SWAPLESS REBALANCING: Check if this should be single-sided
      const direction = swaplessOptions && swaplessOptions.direction;
      const isSwaplessRebalance = swaplessOptions && swaplessOptions.swapless && !!direction;
      const shouldSkipBuffer = isSwaplessRebalance && (
        (direction === 'DOWN' && Y_IS_SOL && lamY.isZero()) ||  // DOWN move = tokens only, no SOL needed
        (direction === 'UP' && X_IS_SOL && lamX.isZero())       // UP move = SOL only, no tokens needed
      );
      
      if (shouldSkipBuffer) {
        console.log(`   🎯 SWAPLESS REBALANCING: Skipping SOL buffer for ${direction} move`);
        console.log(`   • Direction: ${direction} = single-sided position using available tokens only`);
        console.log(`   • No SOL needed from wallet - position will be token-only as intended`);
      } else if (X_IS_SOL) {
        if (lamX.lt(REBALANCE_BUFFER)) {
          // Only get SOL from wallet if NOT in swapless single-sided mode
          console.log(`   ⚠️  Position has insufficient SOL (${(lamX.toNumber() / 1e9).toFixed(6)}) for transaction fees`);
          const walletBalance = new BN(await connection.getBalance(userKeypair.publicKey, 'confirmed'));
          console.log(`   🔍 Wallet SOL balance: ${(walletBalance.toNumber() / 1e9).toFixed(6)} SOL`);
          
          if (walletBalance.lt(REBALANCE_BUFFER)) {
            throw new Error(`Not enough SOL in wallet (${(walletBalance.toNumber() / 1e9).toFixed(6)}) for rebalancing transaction fees (need ${(REBALANCE_BUFFER.toNumber() / 1e9).toFixed(3)})`);
          }
          
          // Add the buffer amount to position from wallet
          lamX = lamX.add(REBALANCE_BUFFER);
          console.log(`   ✅ Added ${REBALANCE_BUFFER.toNumber() / 1e9} SOL from wallet for transaction fees`);
          console.log(`   • Post-Addition lamX: ${lamX.toString()} (${(lamX.toNumber() / 10**dx).toFixed(6)} tokens)`);
        } else {
          lamX = lamX.sub(REBALANCE_BUFFER);
          console.log(`   ✅ Reserved ${REBALANCE_BUFFER.toNumber() / 1e9} SOL from X tokens for transaction fees`);
          console.log(`   • Post-Buffer lamX: ${lamX.toString()} (${(lamX.toNumber() / 10**dx).toFixed(6)} tokens)`);
        }
      } else if (Y_IS_SOL) {
        if (lamY.lt(REBALANCE_BUFFER)) {
          // Only get SOL from wallet if NOT in swapless single-sided mode
          console.log(`   ⚠️  Position has insufficient SOL (${(lamY.toNumber() / 1e9).toFixed(6)}) for transaction fees`);
          const walletBalance = new BN(await connection.getBalance(userKeypair.publicKey, 'confirmed'));
          console.log(`   🔍 Wallet SOL balance: ${(walletBalance.toNumber() / 1e9).toFixed(6)} SOL`);
          ...
          // Add the buffer amount to position from wallet
          lamY = lamY.add(REBALANCE_BUFFER);
          console.log(`   ✅ Added ${REBALANCE_BUFFER.toNumber() / 1e9} SOL from wallet for transaction fees`);
          console.log(`   • Post-Addition lamY: ${lamY.toString()} (${(lamY.toNumber() / 10**dy).toFixed(6)} tokens)`);
        } else {
          lamY = lamY.sub(REBALANCE_BUFFER);
          console.log(`   ✅ Reserved ${REBALANCE_BUFFER.toNumber() / 1e9} SOL from Y tokens for transaction fees`);
          console.log(`   • Post-Buffer lamY: ${lamY.toString()} (${(lamY.toNumber() / 10**dy).toFixed(6)} tokens)`);
        }
      } else {
        const native = new BN(await connection.getBalance(userKeypair.publicKey, 'confirmed'));
        console.log(`   • Native SOL Balance: ${native.toString()} (${(native.toNumber() / 1e9).toFixed(6)} SOL)`);
        if (native.lt(REBALANCE_BUFFER)) throw new Error('Not enough native SOL for rebalancing transaction fees');
        console.log(`   ✅ Sufficient native SOL for transaction fees`);
      }
    }
...
async function recenterPosition(connection, dlmmPool, userKeypair, oldPositionPubKey, originalParams = {}, rebalanceDirection = null) {
  ...
  if (autoCompoundConfig && autoCompoundConfig.enabled) {
    ...
    console.log(`   ✅ FEES ADDED TO POSITION:`);
  ...
  // 4) Use calculated amounts from position + fees (CRITICAL FIX) ──────────────
  ...
  try {
    ...
    if (swaplessConfig && swaplessConfig.enabled && rebalanceDirection) {
      ...
      }, calculatedBalances, false); // Use calculated balances that respect original SOL limit
    } else {
      ...
      openRes = await openDlmmPosition(connection, userKeypair, capitalToUse, tokenRatio, binSpan, poolAddress, liquidityStrategy, null, calculatedBalances, false); // Use calculated balances that respect original SOL limit
    }
