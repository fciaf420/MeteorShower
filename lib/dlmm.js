...
async function recenterPosition(connection, dlmmPool, userKeypair, oldPositionPubKey, originalParams = {}, rebalanceDirection = null) {
  ...
  try {
    ...
    // 🔧 WALLET-AWARE AUTO-COMPOUNDING: Scan wallet for compatible tokens
    const walletTokens = await scanWalletForCompatibleTokens(connection, userKeypair, dlmmPool);
    
    // Add wallet tokens to the compounding (only if they match the LP pair)
    if (walletTokens.tokenXAmount > 0) {
      lamX = lamX.add(walletTokens.walletTokenX);
      console.log(`   💡 Added ${walletTokens.tokenXAmount.toFixed(6)} accumulated tokens from wallet (Token X)`);
      console.log(`   🔒 Verified token mint: ${walletTokens.tokenXMint.slice(0,8)}...`);
    }
    
    if (walletTokens.tokenYAmount > 0) {
      lamY = lamY.add(walletTokens.walletTokenY);
      console.log(`   💡 Added ${walletTokens.tokenYAmount.toFixed(6)} accumulated tokens from wallet (Token Y)`);
      console.log(`   🔒 Verified token mint: ${walletTokens.tokenYMint.slice(0,8)}...`);
    }

    // Prepare exact token amounts from closed position + auto-compounding + wallet tokens
    const exactBalances = {
      lamX: lamX,  // Auto-compounded amounts (position + fees + wallet)
      lamY: lamY
    };
    ...
    console.log(`🎯 Smart wallet-aware auto-compounding result:`);
    ...
    console.log(`   📊 Using position tokens + fees + compatible wallet tokens`)
